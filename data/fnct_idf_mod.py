...
def modify_surface_area(idf, desired_area):
    # Get the Surface object by its name
    surface_obj = idf.getobject('BuildingSurface:Detailed', 'floor')

    # Get the vertices of the current surface
    vertices = get_surface_vertices(surface_obj)

    # Calculate the current surface area
    current_area = calculate_surface_area(vertices)
    ...
    adjacent_surfaces = idf.idfobjects["BuildingSurface:Detailed"]

    for adjacent_surface in adjacent_surfaces:
        if adjacent_surface.Zone_Name == zone_name and adjacent_surface.Name != 'floor':
            adjacent_vertices = get_surface_vertices(adjacent_surface)
...
def modify_window_dimensions(idf, wall_name, window_to_wall_ratio):
    # Get the Wall Surface object by its name
    wall_surface = idf.getobject('BuildingSurface:Detailed', f'wall{wall_name}')
    wall_vertices = get_surface_vertices(wall_surface)

    # Calculate the current wall area
    wall_area = calculate_surface_area(wall_vertices)
    ...
    fenestration_surface = idf.getobject('FenestrationSurface:Detailed', f'window{wall_name}')
    window_vertices = get_surface_vertices(fenestration_surface)

    window_width, window_height = surface_dimensions(window_vertices)
...
def calculate_transmittance(idf, construction):
    ...
    for layer_name in construction.fieldnames[2:]:
        if construction[layer_name] == '':
            continue
        layer_material = idf.getobject('MATERIAL', construction[layer_name])
...
def calculate_new_material_thickness(material, actual_transmittance, target_transmittance, limits=(0.02, 0.3)):
...
def modify_external_wall_transmittance(idf, target_transmittance):
    # Find the construction object with the specified name
    construction = idf.getobject('MATERIAL', 'wall_external')
    if construction is None:
        raise ValueError(f"Construction not found in the IDF file.")

    # Find the material object with the specified name
    material = idf.getobject('MATERIAL', 'insulation_fiberglass_UNI10351')
    if material is None:
        raise ValueError(f"Material not found in the IDF file.")

    # Calculate the current thermal transmittance of the construction
    thermal_transmittance = calculate_transmittance(idf, construction)

    # Calculate the needed thickness for the specified material to achieve the desired transmittance
    new_thickness = calculate_new_material_thickness(material, thermal_transmittance, target_transmittance)
...
def modify_floor_transmittance(idf, target_transmittance):
    # Find the construction object with the specified name
    construction = idf.getobject('MATERIAL', 'floor')
    if construction is None:
        raise ValueError(f"Construction not found in the IDF file.")

    # Find the material object with the specified name
    material = idf.getobject('MATERIAL', 'insulation_extrudedpolystyrene_4cm_UNI10351')
    if material is None:
        raise ValueError(f"Material not found in the IDF file.")
...
def modify_roof_transmittance(idf, target_transmittance):
    # Find the construction object with the specified name
    construction = idf.getobject('MATERIAL', 'roof')
    if construction is None:
        raise ValueError(f"Construction not found in the IDF file.")

    # Find the material object with the specified name
    material = idf.getobject('MATERIAL', 'insulation_extrudedpolystyrene_6cm_UNI10351')
    if material is None:
        raise ValueError(f"Material not found in the IDF file.")
...
def modify_window_u_factor(idf, target_u_factor):
    # Find the construction object with the specified name
    construction = idf.getobject('WINDOWMATERIAL:SIMPLEGLAZINGSYSTEM', 'window_simple')
    if construction is None:
        raise ValueError(f"Construction not found in the IDF file.")
